...
function pickEditable(config: Config): EditableConfig {
  return {
...
function copyEditable(config: EditableConfig): EditableConfig {
  return JSON.parse(JSON.stringify(config)) as EditableConfig
}

function validateRegexList(value: unknown, label: string, errors: string[]): string[] | undefined {
 ...
 */
export function validateConfigPatch(body: unknown): { patch: EditableConfigPatch; errors: string[] } {
  const errors: string[] = []
  ...
  if (include) patch.include = include
  const exclude = validateRegexList(input['exclude'], 'exclude', errors)
  if (exclude) patch.exclude = exclude
  ...
  if (input['events'] !== undefined) {
    ...
    if (events === null || typeof events !== 'object' || Array.isArray(events)) {
      ...
      patch.events = {}
      if (hide) patch.events.hide = hide
      if (show) patch.events.show = show
      if (!hide && !show) delete patch.events
    }
  }
  ...
  if (input['tools'] !== undefined) {
    ...
    if (tools === null || typeof tools !== 'object' || Array.isArray(tools)) {
      ...
      patch.tools = {}
      if (hide) patch.tools.hide = hide
      if (hideCalls) patch.tools.hideCalls = hideCalls
      if (hideResults) patch.tools.hideResults = hideResults
      if (!hide && !hideCalls && !hideResults) delete patch.tools
    }
  }
  ...
  if (input['redact'] !== undefined) {
    ...
    if (!Array.isArray(redact)) {
      ...
      const rules: EditableConfig['redact'] = []
      redact.forEach((rule, i) => {
        ...
        try {
          ...
          errors.push(`redact[${i}]: invalid regex: ${r['pattern'] as string}`)
          return
        }
        rules.push({ pattern: r['pattern'] as string, replace: r['replace'] as string })
      })
      patch.redact = rules
    }
  }
...
function mergeEditable(base: EditableConfig, patch: EditableConfigPatch): EditableConfig {
  const next = copyEditable(base)
  if (patch.include !== undefined) next.include = patch.include
  if (patch.exclude !== undefined) next.exclude = patch.exclude
  if (patch.events !== undefined) {
    if (patch.events.hide !== undefined) next.events.hide = patch.events.hide
    if (patch.events.show !== undefined) next.events.show = patch.events.show
  }
  if (patch.tools !== undefined) {
    if (patch.tools.hide !== undefined) next.tools.hide = patch.tools.hide
    if (patch.tools.hideCalls !== undefined) next.tools.hideCalls = patch.tools.hideCalls
    if (patch.tools.hideResults !== undefined) next.tools.hideResults = patch.tools.hideResults
  }
  if (patch.redact !== undefined) next.redact = patch.redact
  if (patch.sensitiveDefaults !== undefined) next.sensitiveDefaults = patch.sensitiveDefaults
  return next
}

export type ConfigUpdateResult = { ok: true; config: EditableConfig } | { ok: false; errors: string[] }
...
export class MirrorConfigStore {
  private editable: EditableConfig
  ...
  private persist(editable: EditableConfig): void {
   ...
   */
  private readOverrides(): EditableConfigPatch | null {
    let raw: string
    try {
